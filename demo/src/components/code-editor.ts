class CodeEditor extends HTMLElement {
  ...
  #syncLineNumbers(): void {
    ...
    for (const row of sourceRows) {
      ...
      mirrorRows.push(element);
      mirror.appendChild(element);
    }

    this.#gutter.textContent = "";
    for (let index = 0; index < mirrorRows.length; index++) {
      const number = document.createElement("div");
      number.textContent = String(index + 1);
      number.style.height = `${Math.max(mirrorRows[index].offsetHeight, lineHeight)}px`;
      this.#gutter.appendChild(number);
    }
    this.#align();
  }
